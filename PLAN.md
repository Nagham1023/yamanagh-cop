...
**Flagged explicitly, found during pre-PRD-6 hardening (not a surprise to discover at PRD 6 time):** "Rules owned" above lists 15/16/21/22, but as of PRD 5 there is still no wire channel at all for either — `tools/mcp_server.py`'s surface is `receive_hint(text)` and `share_scent_map()` (PRD 4 "Revision 3"), neither of which carries a barrier declaration or a capture claim. Barrier declaration (15/16) has been a documented, un-delivered gap since PRD 1's own `domain/barriers.py` docstring; capture claim/response (21/22) has no channel either. `PRD-6-prep-commit-payload-spec.md` and `tests/unit/test_prd6_missing_wire_channels_guard.py` (`RULE-15-16-BARRIER-DECLARATION-AT-PRD-6`, `RULE-21-22-CAPTURE-CLAIM-AT-PRD-6`) name this precisely — see `PRD-6-security-and-cryptography.md` for the full build.
