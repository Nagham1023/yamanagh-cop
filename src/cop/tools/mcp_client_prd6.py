...
on a timeout or a transient failure anywhere in this module (WIRE-CONTRACT.md's
Status log has the reasoning: a peer may have already applied a non-idempotent
call, so retrying blind risks a duplicate); `orchestrator_peer.py`'s
deadline-guard/technical-loss wrapping (`await_with_deadline`) lives one
layer up, same pattern `request_scent_map`/`send_hint` already used.
