from cop.std_v1.report import build_result_report, final_result, group_details, valid_commit
...
def test_group_details_extracts_the_report_only_fields():
    identity = {
        "group_id": MY_GROUP, "members": ["Nagham"], "repos": {"cop": "url"},
        "mcp_servers": {"cop": "url"}, "llm_model": "template", "spec": {"os": "Windows"},
    }
    details = group_details(identity)
    assert details == {
        "group_id": MY_GROUP, "members": ["Nagham"], "repos": {"cop": "url"},
        "mcp_servers": {"cop": "url"}, "llm_model": "template", "hardware_spec": {"os": "Windows"},
    }
...
def test_build_result_report_shapes_a_full_section_12_report():
    my_identity = {
        "group_id": MY_GROUP, "github_commit": "a" * 40, "members": ["Nagham"],
        "repos": {"cop": "url1"}, "mcp_servers": {"cop": "url1"}, "llm_model": "template", "spec": {},
    }
    their_identity = {
        "group_id": THEIR_GROUP, "github_commit": "b" * 40, "members": ["Yaman"],
        "repos": {"thief": "url2"}, "mcp_servers": {"thief": "url2"}, "llm_model": "claude", "spec": {},
    }
    rows = [
        {"sub_game_number": 1, "result": "capture", "roles": {MY_GROUP: "police", THEIR_GROUP: "thief"},
         "score": {MY_GROUP: 20, THEIR_GROUP: 5}, "winner_group": MY_GROUP},
    ]
    sub_game_meta = [{
    ...
    assert row["steps"] == 12
    assert report["links"]["github"][MY_GROUP] == {"cop": "url1"}
